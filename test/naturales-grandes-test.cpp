 ...
 *                                                                        
 * (Para escribir estas funciones, no se han utilizado conceptos de C++ que no
 * se hayan visto en la asignatura, excepto en las función de conversión de
 * cadena de caracteres a «natural grande» y viceversa, cuyos fundamentos se
 * verán en el tema 10 sobre cadenas de caracteres.)
 *****************************************************************************/

/* 
 * Constante con la que se inicializan las NUM_DIGITOS componentes de un vector
 * que representa números grandes, con el objetivo de maximizar las
 * posibilidades de localizar errores. El valor elegido provoca que cuando
 * «numeroGrande» sea convertido a «string», los valores no inicializados por
 * la función objeto de pruebas se muestren con el carácter '?'
 */
 ...
 *       «numero» y así, sucesivamente, la componente numeroGrande[n-1]
 *       almacena el dígito más significativo de «numero». Las componentes de
 *       índices superiores almacenan el valor 0
 */
...
/*
 * Pre:  En la línea actual de la pantalla se ha escrito un texto anunciando
 *       que se ha realizado una prueba de una determinada función con unos
 *       determinados argumentos.
 * Post: Si los valores de «valorEsperado» y «valorCalculado» son iguales y
 *       todas las componentes de los vectores «testigo1» y «testigo2»
 *       almacenan el valor de la constante «VALOR_INDEFINIDO», completa un
 *       mensaje de la pantalla informando de que la prueba es correcta. En el
 *       caso de que alguna componente de los vectores «testigo1» y «testigo2»
 *       almacene un valor distinto al de la constante «VALOR_INDEFINIDO»,
 *       completa el mensaje de la pantalla indicando que, pese a que el valor
 *       calculado por la función objeto de pruebas sea correcto, esta ha
 *       modificado memoria fuera de los límites del vector.
 *       En el caso de que los valores de «valorEsperado» y «valorCalculado»
 *       no sean iguales, completa el mensaje de la pantalla indicando que la
 *       prueba realizada es incorrecta e informa también de los valores de
 *       «valorEsperado» y «valorCalculado».
 *       Si se ha calculado el resultado correcto, el valor de «todoOk» no se
 *       modifica y en caso contrario, se establece en «false».
 */
void escribirResultadoPrueba(const string valorEsperado,
            const string valorCalculado, const unsigned testigo1[],
            const unsigned testigo2[], bool &todoOk) {
 ...
 *       todas las componentes de los vectores «testigo1» y «testigo2»
 *       almacenan el valor de la constante «VALOR_INDEFINIDO», completa un
 *       mensaje de la pantalla informando de que la prueba es correcta.
 *       En el caso de que alguna componente de los vectores «testigo1» y
 *       «testigo2» almacene un valor distinto al de la constante
 *       «VALOR_INDEFINIDO», completa el mensaje de la pantalla indicando que,
 *       pese a que el valor calculado por la función objeto de pruebas sea
 *       correcto, esta ha modificado memoria fuera de los límites del vector.
 *       En el caso de que los valores de «valorEsperado» y «valorCalculado» no
 *       sean iguales, completa el mensaje de la pantalla indicando que la
 *       prueba realizada es incorrecta e informa también de los valores de
 *       «valorEsperado» y «valorCalculado».
 *       Si se ha calculado el resultado correcto, el valor de «todoOk» no se
 *       modifica y en caso contrario, se establece en «false».
...
void escribirResultadoPrueba(const bool valorEsperado,
            const bool valorCalculado, const unsigned testigo1[],
            const unsigned testigo2[], bool &todoOk) {
    if (valorEsperado == valorCalculado) {
        if (comprobarTestigo(testigo1) && comprobarTestigo(testigo2)) {
            cout << "correcta." << endl;
 ...
 *       que se ha realizado una prueba de una determinada función con unos
 *       determinados argumentos.
 * Post: Si los valores de «valorEsperado» y «valorCalculado» son iguales, los
 *       valores de «desbordaEsperado» y «desbordaCalculado» también son
 *       iguales y todas las componentes de los vectores «testigo1» y
 *       «testigo2» almacenan el valor de la constante «VALOR_INDEFINIDO»,
 *       completa un mensaje de la pantalla informando de que la prueba es
 *       correcta. En el caso de que alguna componente de los vectores
 *       «testigo1» y «testigo2» almacene un valor distinto al de la constante
 *       «VALOR_INDEFINIDO», completa el mensaje de la pantalla indicando que,
 *       pese a que el valor calculado por la función objeto de pruebas sea
 *       correcto, esta ha modificado memoria fuera de los límites del vector.
 *       En el caso de que los valores de «valorEsperado» y «valorCalculado» o
 *       «desbordaEsperado» y «desbordaCalculado» no sean iguales, completa el
 *       mensaje de la pantalla indicando que la prueba realizada es incorrecta
 *       e informa también de los valores de «valorEsperado», «valorCalculado»,
 *       «desbordaEsperado» y «desbordaCalculado», utilizando
 *       «nombreParametro». Si se ha calculado el resultado correcto, el valor
 *       de «todoOk» no se modifica y, en caso contrario, se establece en
 *       «false».
 */
void escribirResultadoPrueba(const string valorEsperado,
            const string valorCalculado, const bool desbordaEsperado,
            const bool desbordaCalculado, const unsigned testigo1[],
            const unsigned testigo2[], const string nombreParametro,
            bool &todoOk) {
    if ((desbordaEsperado || (valorEsperado == valorCalculado))
        ...
        if (comprobarTestigo(testigo1) && comprobarTestigo(testigo2)) {
            ...
            cout << "incorrecta: los valores calculados son los correctos, "
                 << "pero ha modificado memoria fuera de los límites del "
                 << "vector. " << endl;
            todoOk = false;
        }
    } else {
        ...
        cout << "desbordamiento = " << desbordaEsperado << ESCAPE_NORMAL
             << ESCAPE_ROJO << " pero ha calculado " << ESCAPE_ROJO_NEGRITA;
        if (!desbordaEsperado) {
            cout << nombreParametro << " = " << valorCalculado << " y ";
        }
        cout << "desbordamiento = " << desbordaCalculado << ESCAPE_NORMAL
             << ESCAPE_ROJO << '.' << endl;
        cout << ESCAPE_NORMAL;
        todoOk = false;
    }
 ...
 *       «string», los valores no inicializados por la función objeto de
...
void probarConvertir(const unsigned n, const string valorEsperado,
    ...
    unsigned testigo1[NUM_DIGITOS], valorCalculado[NUM_DIGITOS],
             testigo2[NUM_DIGITOS];
...
void probarSumar(const string aStr, const string bStr,
                 ...
                 bool &todoOk) {
    cout << "Prueba sumar (" << aStr << ", " << bStr
         << ", unsigned[], bool &desbordamiento) ";
...
void probarMultiplicar(const string aStr, const unsigned b, 
                       const string valorEsperado, const bool desbordaEsperado,
                       bool &todoOk) {
    cout << "Prueba multiplicar (" << aStr << ", " << b 
         << ", unsigned[], bool &desbordamiento) ";
...
void probarEsCapicua(const string numero, const bool valorEsperado,
    ...
    unsigned testigo1[NUM_DIGITOS], numeroGrande[NUM_DIGITOS],
             testigo2[NUM_DIGITOS];
    convertirDeString(numero, numeroGrande);
    inicializar(testigo1);
    inicializar(testigo2);
